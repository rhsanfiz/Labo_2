...
for arch in ['450.csv','rendija.csv']:#archivos:
    if arch != 'rendija.csv':
        ...
        max_val = max(val)
        serie = -1*val+max_val
        picos,otro = sc.signal.find_peaks(serie,prominence=10,plateau_size=(None,None))#0.20*max_val)
        print(picos)
